...
data RegT = Epsilon | Letter (Int, Char) | Star RegT | Concat (RegT, RegT) | Or (RegT, RegT) deriving (Show, Eq)
...
--all the states that can be reached from the initS state
firstS :: RegT -> [RegT]
firstS t = case t of
    Or (r1, r2) -> firstS r1 ++ firstS r2
    Concat (r1, r2) -> if empty r1 then firstS r1 ++ firstS r2 else firstS r1
    x@(Letter _) -> [x]
    Star r1 -> firstS r1
    Epsilon -> []
...
nextS t leaf = nextS t leaf [] where
  nextS t leaf acc = case t of
        Concat (r1, r2) -> nextS r1 leaf r1' ++ nextS r2 leaf acc where
          r1' = if empty r2 then firstS r2 ++ acc else firstS r2
...
lastS :: RegT -> [RegT]
lastS t = case t of
    Concat (r1, r2) -> if empty r2 then lastS r1 ++ lastS r2 else lastS r2
...
acceptS :: RegT -> [RegT]
acceptS t = if not (empty t) then lastS t else initS : lastS t
